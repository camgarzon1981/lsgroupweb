...
<div>
	<?php echo '<p>'.$user->name.' - '.$user->nit. '</p>'?>
	

			<?php if($hasAccesstoModify){ ?><table>
...
<form method="POST" name="buscadorEmbarquesForm"><table align="center" cellspacing="0" cellpadding="2" bordercolor="#CCCCCC" border="1" width="100%">
			...
			<tr>
								...
								<td colspan="2">Cliente <?php if($hasAccesstoModify){ ?><select name="id_cliente"><option value="">(No seleccionado)</option><?php foreach($empresas_sort as $nit => $nombre){ ?><option value="<?php echo $nit?>" <?php if ($_POST["id_cliente"]==$nit){ echo "selected"; }?>><?php echo $nombre?><?php } ?></select><?php } else { ?><?php echo '<p>'.$user->name.' - '.$user->nit. '</p>'; echo '<input type="hidden" name="id_cliente" id="id_cliente" value="'.$user->nit.'"/>'; }?><input type="hidden" name="accion" id="accion" value="BuscarEmbarquesOperaciones" />
	</td>
			...
			<tr>
				...
				<td><div id="do_div"></div>DO: <input type="text" name="do" id="do" value="<?php echo $parametros['do'] ?>"/></td>
								...
								<td><div id="tipo_div"></div>Tipo: <select name="tipo" id="tipo"><option value="">(No seleccionado)</option><option value="Importacion" <?php if($parametros['tipo']=='Importacion'){?>selected<?php }?>>Importacion</option><option value="Exportacion" <?php if($parametros['tipo']=='Exportacion'){?>selected<?php }?>>Exportacion</option></select></td>
							</tr>
			<tr>
			...
			<tr>
								...
								<td><div id="via_div"></div>Via <select name="via" id="via"><option value="">(No seleccionado)</option><option value="Aerea" <?php if($parametros['via']=='Aerea'){ ?>selected<?php } ?> >Aerea</option><option value="Maritima" <?php if($parametros['via']=='Maritima'){ ?>selected<?php } ?>>Maritima</option><option value="Terrestre" <?php if($parametros['via']=='Terrestre'){?>selected<?php } ?>>Terrestre</option></select></td>
...
<tr>
			...
			<td ><div id="estado_div"></div>Estado: <select name="estado_id" id="estado_id"><option value="">(No seleccionado)</option><option value="Finalizado" <?php if($parametros['estado_id']=='Finalizado'){?>selected<?php }?>>Finalizado</option><option value="No Finalizado" <?php if($parametros['estado_id']=='No Finalizado'){?>selected<?php }?>>No Finalizado</option></select></td>
